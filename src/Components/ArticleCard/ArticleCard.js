...
export const ArticleCard = ({ multimedia, title }) => {
    ...
    // useEffect(() => {
    //     const articleSaved = initialState.savedArticles.find((currentArticle) => currentArticle === title)

    //     if(articleSaved) {
    //         setIsSaved(true)
    //     } else if(!initialState.savedArticles.includes(articleSaved)){
    //         setIsSaved(false)
    //     }
    // }, [title])
    ...
    return(
        ...
        {!initialState.savedArticles.includes(title) && (
            <button onClick={() => saveArticle(title)}>
            <img src={emptyHeart} alt={'save article'} />
        </button>
        )}
